<!DOCTYPE html>
<html lang="vi" x-data="{ sidebarOpen: false }" class="h-full bg-white">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>@yield('title', 'Bảng điều khiển') - {{ config('app.name') }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js']) <!-- Tải các tệp CSS và JS -->
</head>
            ...
            <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
                <div class="flex justify-center items-center h-full">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
                        ...
                        <!-- Thống kê lượt xem -->
                        <div class="bg-black shadow-md rounded-lg p-6 hover:shadow-xl transition duration-300">
                            <h3 class="text-lg font-semibold text-teal-600">Lượt Xem Tổng</h3>
                            <p class="text-3xl font-bold text-teal-600">{{ $newsViewsCount }}</p>
                        </div>

                        <!-- Thống kê lượt bình luận -->
                        <div class="bg-black shadow-md rounded-lg p-6 hover:shadow-xl transition duration-300">
                            <h3 class="text-lg font-semibold text-teal-600">Số Bình Luận</h3>
                            <p class="text-3xl font-bold text-teal-600">{{ $commentsCount }}</p>
                        </div>

                        <!-- Thống kê người dùng -->
                        <div class="bg-black shadow-md rounded-lg p-6 hover:shadow-xl transition duration-300">
                            <h3 class="text-lg font-semibold text-teal-600">Người Dùng</h3>
                            <p class="text-3xl font-bold text-teal-600">{{ $usersCount }}</p>
                        </div>

                        <!-- Thống kê loại tin -->
                        <div class="bg-black shadow-md rounded-lg p-6 hover:shadow-xl transition duration-300">
                            <h3 class="text-lg font-semibold text-teal-600">Loại Tin</h3>
                            <p class="text-3xl font-bold text-teal-600">{{ $categoriesCount }}</p>
                        </div>

                        <!-- Thống kê tin tức -->
                        <div class="bg-black shadow-md rounded-lg p-6 hover:shadow-xl transition duration-300">
                            <h3 class="text-lg font-semibold text-teal-600">Tin Tức</h3>
                            <p class="text-3xl font-bold text-teal-600">{{ $newsCount }}</p>
                        </div>
                    </div>
                </div>
            </main>
